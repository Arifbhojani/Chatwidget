  ...
  this.formShown = false; // 🆕 Show form only once per session
    ...
    init() {
      ...
      if (this.config.theme.button.autoWindowOpen.autoOpen) {
        ...
        // Restore widget state from session (Tawk vs BotStitch active)
        this.restoreWidgetState();
        
        console.log('✅ BotStitch Widget initialized successfully');
    }
  
    // 🆕 Save active widget state to sessionStorage
    saveWidgetState(activeWidget) {
      try {
        sessionStorage.setItem(`botstitch_activeWidget_${this.id}`, activeWidget); // 'bot' or 'tawk'
      } catch (e) {
    ...
    restoreWidgetState() {
      try {
        ...
        if (saved === 'tawk' && window.Tawk_API) {
          ...
          // BotStitch was active, minimize Tawk if present
          this.minimizeTawk();
        }
      } catch (e) {
    ...
    // 🆕 Minimize Tawk widget if present
    minimizeTawk() {
      if (window.Tawk_API && typeof window.Tawk_API.minimize === 'function') {
        try {
          window.Tawk_API.minimize();
        } catch (e) {
          console.warn('Tawk minimize error:', e);
        }
      }
    }
...
submitUserForm() {
  ...
  // Show starter prompts immediately after welcome (if configured)
  if (this.config.theme.chatWindow.starterPrompts && this.config.theme.chatWindow.starterPrompts.length > 0) {
    this.starterPromptsDisplayed = false; // ensure allowed to render now
...
showTransferPrompt() {
  ...
  this.addMessage(promptHTML, 'bot', true);
      ...
      initiateTawkTransfer() {
        ...
        try {
          if (typeof window.Tawk_API.start === 'function') {
...
async triggerHumanTransfer() {
  ...
  try {
   ...
   // NEW: Transfer to human agent
    ...
    // If transfer prompt was shown but user continued chatting, reset cycle
    if (this.config.humanTransfer?.enabled && this.transferPromptShown && !this.isHumanTransferred) {
      this.transferPromptShown = false;
      this.chatStartTime = Date.now();
      if (!this.timerInterval) {
        this.startNewHumanTransferTimer();
    ...
    open() {
      if (this.chatElement) {
        ...
        this.chatElement.style.display = ''; // Ensure visible (undo any hide)
        this.chatElement.classList.add('botstitch-fade-in');
        this.isOpen = true;
        this.hideTooltip();
        // Auto-minimize Tawk when BotStitch opens
        this.minimizeTawk();
        this.saveWidgetState('bot');
          // Always ensure form is present until userInfo exists
          if (this.config.userForm?.enabled && !this.userInfo) {
            const existingUserForm = this.messagesContainer && this.messagesContainer.querySelector('.botstitch-user-form');
            if (!existingUserForm) {
              setTimeout(() => this.showUserForm(), 300);
            }
          } else {
          ...
          if (this.config.features.formCollection?.enabled && !this.formSubmitted) {
            ...
            if (!existingForm) {
              ...
              // Only show starter prompts after user form is completed or not enabled
              if (this.config.theme.chatWindow.starterPrompts.length > 0 && !this.starterPromptsDisplayed) {
                const existingPrompts = document.getElementById(`${this.id}-starter-prompts`);
                if (!existingPrompts) this.showStarterPrompts();
              }
