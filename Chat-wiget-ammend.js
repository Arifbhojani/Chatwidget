...
async function submitMessage(messageText) {
    ...
    // Helper function to safely remove typing indicators
    function safelyRemoveTypingIndicators() {
        // First try to remove the specific indicator if it exists in the correct location
        if (typingIndicator && typingIndicator.parentNode === messagesContainer) {
            messagesContainer.removeChild(typingIndicator);
        } else {
            // Otherwise find and remove all typing indicators
    ...
    // Helper function to ensure chat controls are visible
    function ensureChatControlsVisible() {
        const chatControls = document.querySelector('.chat-controls');
        if (chatControls) {
            chatControls.style.display = 'flex';
            
            // Re-ensure the textarea and send button are properly accessible
            const textarea = chatControls.querySelector('.chat-textarea');
            const sendBtn = chatControls.querySelector('.chat-submit');
    ...
    try {
        ...
        // Always reset the waiting flag
        isWaitingForResponse = false;
        
        // Ensure chat controls are visible and functional
        ensureChatControlsVisible();
        
        // Re-enable auto-resize for textarea
        const textareaElement = document.querySelector('.chat-textarea');
        if (textareaElement) {
            textareaElement.style.height = 'auto';
            textareaElement.addEventListener('input', autoResizeTextarea);
        }
        
        // Optional: log state for debugging
        console.log("Message sent, chat controls restored");
    }
}
